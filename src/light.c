...
static void _light_add_enumerator_device(light_device_enumerator_t *enumerator,
					 light_device_t *new_device)
{
	// Create a new device array
	uint64_t new_num_devices = enumerator->num_devices + 1;
	light_device_t **new_devices =
	    malloc(new_num_devices * sizeof(light_device_t *));
...
static void _light_add_device_target(light_device_t *device,
				     light_device_target_t *new_target)
{
	// Create a new targets array
	uint64_t new_num_targets = device->num_targets + 1;
	light_device_target_t **new_targets =
	    malloc(new_num_targets * sizeof(light_device_target_t *));
...
static void _light_get_target_path(light_context_t *ctx, char *output_path,
				   size_t output_size)
{
...
static void _light_get_target_file(light_context_t *ctx, char *output_path,
				   size_t output_size, char const *file)
{
...
static uint64_t _light_get_min_cap(light_context_t *ctx)
{
	...
	_light_get_target_file(ctx, target_path, sizeof(target_path),
			       "minimum");
...
static light_device_enumerator_t *_light_find_enumerator(light_context_t *ctx,
							 char const *comp)
{
...
static light_device_t *_light_find_device(light_device_enumerator_t *en,
					  char const *comp)
{
...
static light_device_target_t *_light_find_target(light_device_t *dev,
						 char const *comp)
{
...
static bool _light_raw_to_percent(light_device_target_t *target, uint64_t inraw,
				  double *outpercent)
{
...
static bool _light_percent_to_raw(light_device_target_t *target,
				  double inpercent, uint64_t *outraw)
{
	uint64_t max_value = 0;
	if (!target->get_max_value(target, &max_value)) {
		LIGHT_ERR("couldn't read from target");
	...
	double max_value_d = (double)max_value;
	double target_value_d =
	    max_value_d * (light_percent_clamp(inpercent) / 100.0);
	uint64_t target_value =
	    LIGHT_CLAMP((uint64_t)target_value_d, 0, max_value);
	*outraw = target_value;
...
static void _light_print_usage()
{
	printf("Usage:\n"
	       ...
	       "  -T          Multiply brightness by value (can be a non-whole "
	       "number, ignores raw mode)\n"
	       ...
	       "(ignored for -T)\n"
	       "  -s          Specify device target path to use, use -L to "
	       "list available\n"
...
static bool _light_set_context_command(light_context_t *ctx,
				       LFUNCCOMMAND new_cmd)
{
...
static bool _light_parse_arguments(light_context_t *ctx, int argc, char **argv)
{
	...
	while ((curr_arg = getopt(argc, argv, "HhVGSLMNPAUTOIv:s:r")) != -1) {
		...
		case 'v':
			...
			if (log_level < 0 || log_level > 3) {
				fprintf(
				    stderr,
				    "-v argument must be between 0 and 3.\n\n");
				_light_print_usage();
				return false;
			}
		...
		case 'h':
			...
			break;
		case 'V':
			_light_set_context_command(ctx,
		...
		case 'M':
			_light_set_context_command(
			    ctx, light_cmd_get_max_brightness);
			need_target = true;
			break;
		case 'N':
			_light_set_context_command(
		...
		case 'P':
			_light_set_context_command(
			    ctx, light_cmd_get_min_brightness);
			need_target = true;
			break;
		case 'A':
			_light_set_context_command(ctx,
		...
		case 'I':
			_light_set_context_command(
			    ctx, light_cmd_restore_brightness);
			need_target = true;
			break;
		}
	}
	...
	if (need_target) {
		light_device_target_t *curr_target =
		    light_find_device_target(ctx, ctrl_name);
		if (curr_target == NULL) {
			if (specified_target) {
				fprintf(stderr,
					"We couldn't find the specified device "
					"target at the path \"%s\". Use -L to "
					"find one.\n\n",
					ctrl_name);
				return false;
			} else {
				fprintf(
				    stderr,
				    "No backlight controller was found, so we "
				    "could not decide an automatic target. The "
				    "current command will have no effect. "
				    "Please use -L to find a target and then "
				    "specify it with -s.\n\n");
				curr_target = light_find_device_target(
				    ctx, "util/test/dryrun");
			}
		}
	...
	if (need_value || need_float_value) {
		if ((argc - optind) != 1) {
			fprintf(
			    stderr,
			    "please specify a <value> for this command.\n\n");
			_light_print_usage();
			return false;
		}
	}
	...
	if (need_value) {
		if (ctx->run_params.raw_mode) {
			if (sscanf(argv[optind], "%lu",
				   &ctx->run_params.value) != 1) {
				fprintf(stderr,
			...
			if (!_light_percent_to_raw(
				ctx->run_params.device_target, percent_value,
				&raw_value)) {
				LIGHT_ERR("failed to convert from percent to "
					  "raw for device target");
				return false;
			}
	...
	if (need_float_value) {
		if (sscanf(argv[optind], "%f", &ctx->run_params.float_value) !=
		    1) {
...
light_context_t *light_initialize(int argc, char **argv)
{
	...
	uid_t euid = geteuid();
	gid_t egid = getegid();
	// If the real user ID is different from the effective user ID (SUID
	// mode) and if we have the effective user ID of root (0) and if the
	// effective group ID is different from root (0), then make sure to set
	// the effective group ID to root (0).
	if ((uid != euid) && (euid == 0) && (egid != 0)) {
		if (setegid(euid) < 0) {
			LIGHT_ERR("could not change egid from %u to %u (uid: "
				  "%u, euid: %u)",
				  egid, euid, uid, euid);
			return false;
		}
	...
	// If we are root, use the system-wide configuration folder, otherwise
	// try to find a user-specific folder, or fall back to ~/.config
	if (euid == 0) {
		snprintf(new_ctx->sys_params.conf_dir,
			 sizeof(new_ctx->sys_params.conf_dir), "%s",
			 "/etc/light");
	} else {
		...
		if (xdg_conf != NULL) {
			snprintf(new_ctx->sys_params.conf_dir,
				 sizeof(new_ctx->sys_params.conf_dir),
				 "%s/light", xdg_conf);
		} else {
			snprintf(new_ctx->sys_params.conf_dir,
				 sizeof(new_ctx->sys_params.conf_dir),
				 "%s/.config/light", getenv("HOME"));
		}
	}

	// Make sure the configuration folder exists, otherwise attempt to
	// create it
	int32_t rc = light_mkpath(new_ctx->sys_params.conf_dir,
	...
	light_create_enumerator(new_ctx, "sysfs", &impl_sysfs_init,
				&impl_sysfs_free);
	light_create_enumerator(new_ctx, "util", &impl_util_init,
				&impl_util_free);
	light_create_enumerator(new_ctx, "razer", &impl_razer_init,
				&impl_razer_free);

	// This is where we would create enumerators from plugins as well
	// 1. Run the plugins get_name() function to get its name
	// 2. Point to the plugins init() and free() functions when creating the
	// enumerator

	// initialize all enumerators, this will create all the devices and
	// their targets
...
light_device_enumerator_t *light_create_enumerator(light_context_t *ctx,
						   char const *name,
						   LFUNCENUMINIT init_func,
						   LFUNCENUMFREE free_func)
{
	// Create a new enumerator array
	uint64_t new_num_enumerators = ctx->num_enumerators + 1;
	light_device_enumerator_t **new_enumerators =
	...
	new_enumerators[ctx->num_enumerators] =
	    malloc(sizeof(light_device_enumerator_t));
	light_device_enumerator_t *returner =
	    new_enumerators[ctx->num_enumerators];
...
bool light_init_enumerators(light_context_t *ctx)
{
	bool success = true;
	for (uint64_t i = 0; i < ctx->num_enumerators; i++) {
		light_device_enumerator_t *curr_enumerator =
		    ctx->enumerators[i];
...
bool light_free_enumerators(light_context_t *ctx)
{
	...
	for (uint64_t i = 0; i < ctx->num_enumerators; i++) {
		...
		if (curr_enumerator->devices != NULL) {
			for (uint64_t d = 0; d < curr_enumerator->num_devices;
			     d++) {
				light_delete_device(
				    curr_enumerator->devices[d]);
			}
...
light_device_target_t *light_find_device_target(light_context_t *ctx,
						char const *name)
{
	light_target_path_t new_path;
	if (!light_split_target_path(name, &new_path)) {
		LIGHT_WARN("light_find_device_target needs a path in the "
			   "format of \"enumerator/device/target\", the "
			   "following format is not recognized:  \"%s\"",
			   name);
		return NULL;
	}
	...
	*/
	...
	light_device_enumerator_t *enumerator =
	    _light_find_enumerator(ctx, new_path.enumerator);
	if (enumerator == NULL) {
	...
	light_device_t *device =
	    _light_find_device(enumerator, new_path.device);
	if (device == NULL) {
		LIGHT_WARN("no such device, \"%s\"", new_path.device);
		return NULL;
	}

	light_device_target_t *target =
	    _light_find_target(device, new_path.target);
	if (target == NULL) {
...
bool light_cmd_list_devices(light_context_t *ctx)
{
	...
	for (uint64_t enumerator = 0; enumerator < ctx->num_enumerators;
	     enumerator++) {
		light_device_enumerator_t *curr_enumerator =
		    ctx->enumerators[enumerator];
		for (uint64_t device = 0; device < curr_enumerator->num_devices;
		     device++) {
			light_device_t *curr_device =
			    curr_enumerator->devices[device];
			for (uint64_t target = 0;
			     target < curr_device->num_targets; target++) {
				light_device_target_t *curr_target =
				    curr_device->targets[target];

				printf("\t%s/%s/%s\n", curr_enumerator->name,
				       curr_device->name, curr_target->name);
			}
		}
	}
...
bool light_cmd_get_brightness(light_context_t *ctx)
{
	...
	if (ctx->run_params.raw_mode) {
		...
		if (!_light_raw_to_percent(target, value, &percent)) {
			LIGHT_ERR("failed to convert from raw to percent from "
				  "device target");
			return false;
		}
		printf("%.2f\n", percent);
...
bool light_cmd_set_min_brightness(light_context_t *ctx)
{
	...
	int32_t rc =
	    light_mkpath(target_path, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	if (rc && errno != EEXIST) {
		LIGHT_ERR(
		    "couldn't create target directory for minimum brightness");
		return false;
	}

	char target_filepath[NAME_MAX];
	_light_get_target_file(ctx, target_filepath, sizeof(target_filepath),
			       "minimum");
...
bool light_cmd_get_min_brightness(light_context_t *ctx)
{
	...
	if (ctx->run_params.raw_mode) {
		...
		if (!_light_raw_to_percent(ctx->run_params.device_target,
					   minimum_value, &minimum_d)) {
			LIGHT_ERR("failed to convert value from raw to percent "
				  "for device target");
			return false;
		}
...
bool light_cmd_save_brightness(light_context_t *ctx)
{
	...
	if (rc && errno != EEXIST) {
		LIGHT_ERR(
		    "couldn't create target directory for save brightness");
		return false;
	}
	...
	if (!ctx->run_params.device_target->get_value(
		ctx->run_params.device_target, &curr_value)) {
...
light_device_t *light_create_device(light_device_enumerator_t *enumerator,
				    char const *name, void *device_data)
{
...
light_device_target_t *
light_create_device_target(light_device_t *device, char const *name,
			   LFUNCVALSET setfunc, LFUNCVALGET getfunc,
			   LFUNCMAXVALGET getmaxfunc, LFUNCCUSTOMCMD cmdfunc,
			   void *target_data)
{
	light_device_target_t *new_target =
	    malloc(sizeof(light_device_target_t));
