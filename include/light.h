...
/* Describes a target within a device (for example a led on a keyboard, or a
 * controller for a backlight) */
struct _light_device_target_t {
  char name[256];
...
/* An enumerator that is responsible for creating and freeing devices as well as
 * their targets */
struct _light_device_enumerator_t {
  char name[256];
  LFUNCENUMINIT init;
...
// A command that can be run (set, get, add, subtract, print help, print
// version, list devices etc.)
typedef bool (*LFUNCCOMMAND)(light_context_t *);

struct _light_context_t {
  struct {
    LFUNCCOMMAND command; // What command was issued
    // Only one of value and raw_value is populated; which one depends on the
    // command
...
bool light_split_target_path(char const *in_path,
                             light_target_path_t *out_path);

/* Returns the found device target, or null. Name should be
 * enumerator/device/target */
